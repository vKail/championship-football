...
//    }
    public Match updateStatusMatch(long id, String status){
        ...
        return matchRepository.updateMatchByStatus(id, updatedMatch);
    }
    public String updateResult(long id){
        Match updatedMatch = findMatchById(id);
        List<Goal> goals = goalService.getGoalByMatch(updatedMatch);
        int goalTeamOne = 0;
        int goalsTeamTwo = 0;
        String result;
        for (Goal goal : goals) {
        ...
        return matchRepository.updateMatchByResult(id, updatedMatch);
    }
